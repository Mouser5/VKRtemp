...
def objective_rusher(trial):
    ...
    avg_gold_per_turn = total_gold_found / (
        total_turns / 2
    )  # Делим на 2, т.к. turn_number общий
    ...
    if win_rate >= 0.85:
        print(
            f"Попытка {trial.number} Результат:    {rusher_score:.3f} = {win_rate:.3f} + {avg_gold_per_turn * 2.0:.3f}"
        )
