...
def objective_control(trial):
    params = {
        ...
        "sabotage_pickaxe_weight": trial.suggest_float("sabotage_pickaxe_weight", 2.0, 20.0),
        "rockfall_base": trial.suggest_float("rockfall_base", 10.0, 50.0),
        "rockfall_ladder_bonus": trial.suggest_float("rockfall_ladder_bonus", 20.0, 80.0),
        "rockfall_gold_penalty": trial.suggest_float("rockfall_gold_penalty", 10.0, 100.0),
        "rockfall_own_path_penalty": trial.suggest_float("rockfall_own_path_penalty", 10.0, 100.0),

        "max_kept_sabotages": trial.suggest_int("max_kept_sabotages", 1, 4),  # Разрешаем копить поломки!
        "max_kept_repairs": trial.suggest_int("max_kept_repairs", 1, 3),
        "ladder_bonus": trial.suggest_float("ladder_bonus", 5.0, 50.0),  # Лестницы важны для контроля
    ...
    broken_time_ratio = total_opp_broken_turns / (total_turns / 2)  # Процент времени, когда враг сломан
    ...
    if win_rate >= 0.85:
        print(f'Попытка {trial.number} Результат:    {control_score:.3f} = {win_rate:.3f} + {broken_time_ratio * 0.5:.3f} - {avg_opp_gold*0.1}')
